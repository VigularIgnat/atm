<?php 
    ...
    if (!(isset($_POST["num_card"]))|| !(isset($_POST["card_pin"]))){
       ...
       die("No");
        ...
        /*$query_course="SELECT course, valute
        FROM  courses
        WHERE valute!='UAN'";
        $result_course = mysqli_query($link, $query);
        
        $n_course=mysqli_num_rows($result_course);*/    
        if ($n ==1 ) {
            ...
            $page_client=str_replace("{balance}",round($row["balance"],1), $page_client);
            ...
            /*for ($i=0; $i<$n_course;$i++){
                ...
                $page_client=str_replace("{valute1}",$row_course["valute"], $page_client);
                $page_client=str_replace("{course1}",$row_course["course"], $page_client);
            }*/
            if ($row["status"]==1){
